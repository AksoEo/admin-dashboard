...
export const FILTERS = {
    ...
    tags: {
        ...
        editor ({ value, onChange, onEnabledChange, hidden, userData }) {
            ...
            return (
                <ul class={'congress-locations-tags-filter' + (value.length ? ' has-items' : '')}>
                    {value.map((item, i) => (
                        <li class="tag-item" key={i}>
                            <Button class="remove-button" icon small disabled={hidden} onClick={() => {
...
function TagItem ({ congress, instance, id }) {
    const [loading, error, tag] = useDataView('congresses/locationTag', {
...
function TagPicker ({ open, onClose, value, onChange, congress, instance }) {
    return (
        <ItemPicker
            ...
            search={{ fields: 'name', placeholder: locale.search.filters.tagPicker.search }}
            emptyLabel={locale.search.filters.tagPicker.empty}
            fields={TAG_FIELDS}
            locale={{}}
            value={value}
            onChange={onChange} />
